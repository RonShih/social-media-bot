...
5. **Last-week metrics** (only if a previous plan exists at `reports/plans/<brand>/<prev iso_week>/plan.json` with at least one `Schedule.posted_url`):
   - Read `.claude/skills/weekly-plan-review/SKILL.md`.
   - Main session runs the Playwright scrape (logged-in, own posts only). Wrap each Playwright call into the action log.
...
6. **Research per platform** (subagents, parallel, one per requested platform — by default all 6):
   - For each platform spawn `subagent_type: general-purpose`:
     - Prompt: "Read `.claude/skills/weekly-plan-research/SKILL.md`. Run own-post analysis via Playwright with the brand's logged-in session for `socials.<platform>.url`; for competitors, spawn `fetch-reference` subagents (one per competitor URL) and synthesize. Return JSON exactly per SKILL.md output."
     - **Caveat**: Playwright cannot reach a subagent — pass own-post analysis to the main session, OR have the subagent return only competitor analysis and add a flag `needs_own_scrape: true`. Recommended: subagent does competitor research only; main session then loops own-post analysis platform-by-platform sequentially (single browser, brand-logged-in, action-logged).
...
7. **Schedule** (main session, no subagent — pure compute):
   ...
   - Output: `frequency[]` + `schedule[]` for the week.

8. **Drafts** (subagents, parallel — one per slot):
   - For each `slot` in `schedule[]`, spawn one `weekly-plan-draft` subagent.
   - Pass: `slot`, `research_rows` filtered to that platform, `brand_yaml`, `reference` (if applicable).
   - Subagent prompt: "Read `.claude/skills/weekly-plan-draft/SKILL.md`. Return JSON exactly per output schema."
   - Aggregate all draft results back into `schedule[]` rows (caption, hashtags, media_path, reference_urls, draft_id, rationale).
...
11. **Reply** (channel: TG `reply`; terminal: main-session text). Plain text:
    ...
    plan.xlsx: reports/plans/<brand>/<iso_week>/plan.xlsx
    last week's metrics: <updated | none>

    review the plan, then run /publish-from-plan --day today (or any weekday).
    ```

## Verification

- [ ] `reports/plans/<brand>/<iso_week>/plan.json` and `plan.xlsx` exist.
- [ ] `Schedule` has at least one slot per requested platform.
- [ ] `Research` is non-empty for at least the platforms with reachable own accounts.
